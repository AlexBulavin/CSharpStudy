...
public class MyLibrary
{
    ...
    // Метод создания двумерного массива string
    /// <summary> Метод создания одномерного массива double </summary>
    /// <param name="count">Количество элементов нового double массива</param>
    /// <returns>Созданный массив из count элементов типа double</returns>
    /// <example> double[] arr = CreateArrayDouble(count) </example>
    public static string[,] CreateArrayStringTwo(int m, int n)
    {
    ...
    /// <param name="rows">Количество строк нового double массива</param>    
    /// <returns>Созданный массив из count элементов типа double</returns>
    /// <example> double[,] arr = CreateArrayDouble(rows, columns) </example>
    public static double[,] CreateArrayDouble(int rows, int columns)
    {
        return new double[rows, columns]; // new double[count];
    }
    ...
    // Метод подготовки для печати двумерного double массива !ПЛОХОЙ WriteLine(PrintGoodTwo(arrName, 4))
    /// <summary> Метод подготовки переменной типа string для печати. Элементы имеют размерность interval пунктов. </summary>
    /// <param name="array">Массив, элементы которого нужно вывести на печать</param>
    /// <returns>Строка, заполненная элементами массива с интервалом interval пунктов</returns>
    /// <example> Пример вызова метода: WriteLine(PrintGood(arrName, 4)) </example> 
    public static string PrintGoodTwo(double[,] array, int interval)
    {
        string output = String.Empty; //Создали пустую строку
        int size0 = array.GetLength(0);
        int size1 = array.GetLength(1);
    ...
    {
        string output = String.Empty; //Создали пустую строку
        int size0 = array.GetUpperBound(0);

        for (int i = 0; i < size0; i++)
        {
            int size1 = array[i].GetUpperBound(0);
            for (int j = 0; j < size1; j++)
                output += $"{array[i][j]}{string.Concat(Enumerable.Repeat("\t", interval))}";
            output += "\n";
        }
        ...
        public static string PrintGoodTwo(ulong[][] array, int interval)
    {
        string output = String.Empty; //Создали пустую строку
        int size0 = array.GetUpperBound(0);
    ...
    /// <returns>Строка, заполненная элементами массива с интервалом interval пунктов</returns>
    /// <example> Пример вызова метода: WriteLine(PrintGood(arrName, 4)) </example> 
    public static string PrintGoodTwo(string[,] array, int interval)
    {
        string output = String.Empty; //Создали пустую строку
        int size0 = array.GetLength(0);
        int size1 = array.GetLength(1);
    ...
    /// <param name="text">Текст пояснения для пользователя - что он должен ввести</param>
    /// <returns>Введенное пользователем значение</returns>
    public static ulong InputUlong(string text, bool DEBUG)
    {
        OutputDynamicString(text);
    ...
    /// <param name="text">Текст пояснения для пользователя - что он должен ввести</param>
    /// <param name="dob">Параметр подтверждает, что вводимое число должно быть double</param>
    /// <returns>Введенное пользователем значение или 0.0, если dob = false</returns>
    public static double Input(string text, bool dob)
    {
        if (dob)
        {
            OutputDynamicString(text);
    ...
    public static int FillRandStringTwo(string[,] array, bool DEBUG)
    {
        ...
        //int[] letters = new int[175-128 + 224-241 + 1];//128 - 175, 224-241, 33 - диапазон кириллических букв (в десятичном представлении) и восклицательный знак
        string alphabet = "АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЬЫЭЮЯабвгдеёжзийклмнопрстуфхцчшщьыъэюя!";
        bool trigger = false;
        int counter = 0;
        char sign = '!';
        ...
        for (int i = 0; i < size0; i++)
        {
            for (int j = 0; j < size1; j++)
            {
                ...
                for (int m = 0; m < Random.Shared.Next(2, 6); m++)
                {
